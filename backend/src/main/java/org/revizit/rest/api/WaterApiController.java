...
public class WaterApiController implements WaterApi {
    ...
    private final WaterApiDelegate delegate;

    public WaterApiController(@Autowired(required = false) WaterApiDelegate delegate) {
        this.delegate = Optional.ofNullable(delegate).orElse(new WaterApiDelegate() {});
    }

    @Override
    public WaterApiDelegate getDelegate() {
        return delegate;
    }
